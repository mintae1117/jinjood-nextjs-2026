...
- **원칙**: 데이터 의존 섹션만 로딩 게이트 안에 두고, 정적 섹션(`AboutSection`·`VideoSection`·`SNSSection`·`LocationSection`)은 항상 렌더한다.
...
- **h1은 페이지당 하나**: 홈은 `AboutSection`, 목록은 `PageHeader`, 상세는 `ProductDetail`의 `ProductName`. `HeroBanner`는 h2다.
